	...
	if (GetWorld()->GetName() == FString("EndLevel") && !IsRealEnd)
	{
		...
		if(bUsingSniper==false)
		{
			...
			if(CanShoot&&isLMBC)
			{
				...
				if(isRifleZooming)
				{
					FVector startLoc = ScopeCamLocMesh->GetComponentLocation();
				...
				if(isRifleShootableC)
				{
					...
					if(isRifleZooming)
					{
						...
						if(isZoomingLineTraceHitted)
						{
							...
							if(animInst)
							{
								...
								{
									auto fireSocketLoc = rifleComp->GetSocketTransform(FName("RifleFirePosition")).GetLocation();
									auto niagara = UNiagaraFunctionLibrary::SpawnSystemAtLocation(GetWorld(), BulletTrailSystem, notZoomingHitResult.Location, FRotator::ZeroRotator,FVector(1), true, true, ENCPoolMethod::AutoRelease);
									if(niagara)
									{
										niagara->SetVectorParameter(FName("EndPoint"), fireSocketLoc);
									}
						...
						else
						{
							...
							if(isRifleZooming)
							{
								...
								auto niagara = UNiagaraFunctionLibrary::SpawnSystemAtLocation(GetWorld(), BulletTrailSystem, ForwardLoc, FRotator::ZeroRotator, FVector(1), true, true, ENCPoolMethod::AutoRelease);
								if(niagara)
								{
					...
					else
					{
						...
						// 줌을 하고 있지 않을 때 라인 트레이스가 적중하지 않았다면
						else
						{
							auto randF = UKismetMathLibrary::RandomFloatInRange(-0.4, -1.5);
							auto randF2 = UKismetMathLibrary::RandomFloatInRange(-0.5, 0.5);
